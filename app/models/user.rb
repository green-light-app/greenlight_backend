class User < ApplicationRecord
  ...
  has_many :matches
  has_many :inverse_matches, foreign_key: :crush_id, class_name: Match
  has_one :info

  def all_matches
    matches_array = matches.map{|match| match.crush if match.confirmed}
    matches_array + inverse_matches.map{|match| match.user if match.confirmed}
    matches_array.compact
  end

  def pending_matches
    matches.map{|match| match.crush if !match.confirmed}.compact
  end
